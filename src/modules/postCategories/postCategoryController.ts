...
export class PostCategoryController {
  ...
  async create(req: Request, res: Response) {
    try {
      ...
      // If seoMeta came as a JSON string (multipart/form-data), parse it safely
      if (typeof seoMeta === 'string') {
        try {
          seoMeta = JSON.parse(seoMeta)
        } catch (err) {
          // ignore parse error and treat as undefined
          seoMeta = undefined
        }
      }

      // Support two shapes:
      // - seoMeta as JSON string/object (older clients)
      // - individual fields submitted as multipart form (seoTitle, metaDescription, ...)
      if (typeof seoMeta === 'string') {
        try {
          seoMeta = JSON.parse(seoMeta)
        } catch (err) {
      ...
      if (
        ...
        // Merge individual fields when provided (multipart/form-data)
        if (!processedSeoMeta) processedSeoMeta = {}
        if (seoTitle) processedSeoMeta.seoTitle = seoTitle
        if (metaDescription) processedSeoMeta.metaDescription = metaDescription
        if (canonicalUrl) processedSeoMeta.canonicalUrl = canonicalUrl
        if (focusKeyword) processedSeoMeta.focusKeyword = focusKeyword
        if (ogType) processedSeoMeta.ogType = ogType
        if (typeof noindex !== 'undefined')
          processedSeoMeta.noindex = typeof noindex === 'string' ? noindex === 'true' : !!noindex
        if (typeof nofollow !== 'undefined')
          processedSeoMeta.nofollow = typeof nofollow === 'string' ? nofollow === 'true' : !!nofollow

        // Multer single('seoImage') places the file at req.file
  ...
  async update(req: Request, res: Response) {
    try {
      ...
      // Handle file upload for seoImage and accept individual fields
      let processedSeoMeta: SeoDto | undefined = undefined
