...
export class PostCategoryController {
  ...
  // Helper: parse seo meta from multipart/form-data request
  // Helper: build SeoDto from discrete form fields + optional uploaded file
  // NOTE: we intentionally do NOT accept a stringified `seoMeta` JSON field anymore.
  private parseSeoMetaFromRequest(req: Request): SeoDto | undefined {
    const { seoTitle, metaDescription, canonicalUrl, focusKeyword, ogType, noindex, nofollow } = (req.body || {}) as any

    // If there's no seo-related input at all, return undefined
    const hasSeoFields = seoTitle || metaDescription || canonicalUrl || focusKeyword || ogType || typeof noindex !== 'undefined' || typeof nofollow !== 'undefined' || (req as any).file
    ...
    if (typeof nofollow !== 'undefined') processed.nofollow = typeof nofollow === 'string' ? nofollow === 'true' : !!nofollow

    const seoImageFile = (req as any).file
    if (seoImageFile) {
      processed.seoImage = {
        buffer: seoImageFile.buffer || seoImageFile.data,
        originalName: seoImageFile.originalname || seoImageFile.name
      }
    }
  ...
  async update(req: Request, res: Response) {
    try {
      ...
      const { id } = req.params
      const body: any = req.body || {}
      const { name, slug, description, parentId, active } = body
