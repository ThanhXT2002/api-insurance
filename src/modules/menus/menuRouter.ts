 ...
 *     description: |
 *       Lấy danh sách menu items theo categoryId với tree structure.
 *       Filter theo active status:
 *       - `active=true`: Chỉ lấy items có active=true
 *       - `active=false`: Chỉ lấy items có active=false
 *       - Không truyền active: Lấy tất cả items (cả active và inactive)
 *     parameters:
 *       - in: path
 *         name: categoryId
 *         required: true
 ...
 *           type: boolean
 *           default: true
 *         description: Có include children hay không (tree structure)
 *     responses:
 *       200:
 ...
 *                 message:
 *                   type: string
 *                   example: "Lấy menu items thành công"
 *                 data:
 *                   type: array
 *                   items:
 *                     type: object
...
router.get(
  ...
  authenticate,
  requirePermissions('menu.view'),
  itemController.getByCategory.bind(itemController)
)
...
router.post(
  ...
  authenticate,
  requirePermissions('menu.update'),
  itemController.batchActive.bind(itemController)
)
...
router.post(
  ...
  authenticate,
  requirePermissions('menu.update'),
  itemController.reorder.bind(itemController)
)
...
router.post(
  ...
  authenticate,
  requirePermissions('menu.update'),
  itemController.batchUpdateOrder.bind(itemController)
)
...
router.post(
  ...
  authenticate,
  requirePermissions('menu.update'),
  itemController.moveItem.bind(itemController)
)
...
router.post(
  ...
  authenticate,
  requirePermissions('menu.create'),
  itemController.duplicate.bind(itemController)
)
...
router.get(
  ...
  authenticate,
  requirePermissions('menu.view'),
  itemController.getChildren.bind(itemController)
)
 ...
 */
router.get(
  '/items/:id/count-children',
  authenticate,
  requirePermissions('menu.view'),
  itemController.countChildren.bind(itemController)
)
